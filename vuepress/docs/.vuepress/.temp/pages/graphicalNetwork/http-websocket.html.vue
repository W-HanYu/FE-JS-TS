<template><div><h1 id="既然有-http-协议-为什么还要有-websocket" tabindex="-1"><a class="header-anchor" href="#既然有-http-协议-为什么还要有-websocket" aria-hidden="true">#</a> 既然有 HTTP 协议，为什么还要有 WebSocket？</h1>
<p><strong>求知，好学，勤奋</strong>，这些刻在你 DNA 里的东西都动起来了。</p>
<p>你点开后发现。</p>
<p>长相平平无奇的古某提示你&quot;道士 9 条狗，全服横着走&quot;。</p>
<p>影帝某辉老师跟你说&quot;系兄弟就来砍我&quot;。</p>
<p><img src="https://img-blog.csdnimg.cn/img_convert/b8cca4b1291f25235bc8df3dddbb6da3.png" alt="图片"></p>
<p>来都来了，你就选了个角色进到了游戏界面里。</p>
<p><img src="https://img-blog.csdnimg.cn/img_convert/95e5b4cee384b182d0e604378c3ca00a.jpeg" alt="图片"></p>
...
<h2 id="使用-http-不断轮询" tabindex="-1"><a class="header-anchor" href="#使用-http-不断轮询" aria-hidden="true">#</a> 使用 HTTP 不断轮询</h2>
...
<p><img src="https://img-blog.csdnimg.cn/img_convert/5e0e0e25e8aca80812c9a2892032111c.png" alt="图片"></p>
<p>那么问题又来了，<strong>有没有更好的解决方案？</strong></p>
<p>有，而且实现起来成本还非常低。</p>
<h2 id="长轮询" tabindex="-1"><a class="header-anchor" href="#长轮询" aria-hidden="true">#</a> 长轮询</h2>
...
<p><img src="https://img-blog.csdnimg.cn/img_convert/1058a96ba35215c0f30accc3ff5bb824.png" alt="图片"></p>
<p>比如，某度云网盘就是这么干的。所以你会发现一扫码，手机上点个确认，电脑端网页就<strong>秒跳转</strong>，体验很好。</p>
<p><img src="https://img-blog.csdnimg.cn/img_convert/a3a8c95b97d2bac26cfab123a4da68b2.png" alt="图片"></p>
<p>像这种发起一个请求，在较长时间内等待服务器响应的机制，就是所谓的<strong>长轮询机制</strong>。我们常用的消息队列 RocketMQ 中，消费者去取数据时，也用到了这种方式。</p>
<p><img src="https://img-blog.csdnimg.cn/img_convert/6173c1d25abc914ef17ee9e534ed6a5f.png" alt="图片"></p>
...
<p>而现在使用最广泛的<code v-pre>HTTP/1.1</code>，也是基于TCP协议的，<strong>同一时间里</strong>，客户端和服务器<strong>只能有一方主动</strong>发数据，这就是所谓的<strong>半双工</strong>。</p>
...
<p><img src="https://img-blog.csdnimg.cn/img_convert/3bbe4c5db972513f912d30ba8cbddd65.png" alt="图片"></p>
<h3 id="怎么建立websocket连接" tabindex="-1"><a class="header-anchor" href="#怎么建立websocket连接" aria-hidden="true">#</a> 怎么建立WebSocket连接</h3>
...
<div class="language-http ext-http line-numbers-mode"><pre v-pre class="language-http"><code><span class="token header"><span class="token header-name keyword">Connection</span><span class="token punctuation">:</span> <span class="token header-value">Upgrade</span></span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这些 header 头的意思是，浏览器想<strong>升级协议（Connection: Upgrade）</strong>，并且<strong>想升级成 WebSocket 协议（Upgrade: WebSocket）</strong>。同时带上一段<strong>随机生成的 base64 码（Sec-WebSocket-Key）</strong>，发给服务器。</p>
<p>如果服务器正好支持升级成 WebSocket 协议。就会走 WebSocket 握手流程，同时根据客户端生成的 base64 码，用某个<strong>公开的</strong>算法变成另一段字符串，放在 HTTP 响应的 <code v-pre>Sec-WebSocket-Accept</code> 头里，同时带上<code v-pre>101状态码</code>，发回给浏览器。HTTP 的响应如下：</p>
<div class="language-http ext-http line-numbers-mode"><pre v-pre class="language-http"><code><span class="token response-status"><span class="token http-version property">HTTP/1.1</span> <span class="token status-code number">101</span> <span class="token reason-phrase string">Switching Protocols\r\n</span></span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>HTTP 状态码=200（正常响应）的情况，大家见得多了。101 确实不常见，它其实是指<strong>协议切换</strong>。</p>
<p><img src="https://img-blog.csdnimg.cn/img_convert/dea71991b336c876cae2e1ebdf03b62d.png" alt="图片"></p>
<p>之后，浏览器也用同样的<strong>公开算法</strong>将<code v-pre>base64码</code>转成另一段字符串，如果这段字符串跟服务器传回来的<strong>字符串一致</strong>，那验证通过。</p>
<p><img src="https://img-blog.csdnimg.cn/img_convert/117eebe06cc6b35ded3216a95706f080.png" alt="图片"></p>
<p>就这样经历了一来一回两次 HTTP 握手，WebSocket就建立完成了，后续双方就可以使用 webscoket 的数据格式进行通信了。</p>
<p><img src="https://img-blog.csdnimg.cn/img_convert/f4edd3018914fe6eb38fad6aa3fd2d65.png" alt="图片"></p>
<h3 id="websocket抓包" tabindex="-1"><a class="header-anchor" href="#websocket抓包" aria-hidden="true">#</a> WebSocket抓包</h3>
<p>我们可以用wireshark抓个包，实际看下数据包的情况。</p>
<p><img src="https://img-blog.csdnimg.cn/img_convert/f756ca625523f0f9d40a402465179bbe.png" alt="图片"></p>
<p>上面这张图，注意画了红框的第<code v-pre>2445</code>行报文，是WebSocket的<strong>第一次握手</strong>，意思是发起了一次带有<code v-pre>特殊Header</code>的HTTP请求。</p>
<p><img src="https://img-blog.csdnimg.cn/img_convert/82d65f08dad05e6b537ea06b94224a5f.png" alt="图片"></p>
<p>上面这个图里画了红框的<code v-pre>4714</code>行报文，就是服务器在得到第一次握手后，响应的<strong>第二次握手</strong>，可以看到这也是个 HTTP 类型的报文，返回的状态码是 101。同时可以看到返回的报文 header 中也带有各种<code v-pre>WebSocket</code>相关的信息，比如<code v-pre>Sec-WebSocket-Accept</code>。</p>
<p><img src="https://img-blog.csdnimg.cn/img_convert/382c7699530ea7e7b22f60bb68af21bd.png" alt="图片"></p>
...
<p><img src="https://img-blog.csdnimg.cn/img_convert/2e9d4b1652bdfa1e3ae4bb24f70a1b5a.png" alt="图片"></p>
<p>这就有点&quot;<strong>借壳生蛋</strong>&quot;的那意思。</p>
<p>HTTP和WebSocket的关系</p>
<h3 id="websocket的消息格式" tabindex="-1"><a class="header-anchor" href="#websocket的消息格式" aria-hidden="true">#</a> WebSocket的消息格式</h3>
<p>上面提到在完成协议升级之后，两端就会用webscoket的数据格式进行通信。</p>
<p>数据包在WebSocket中被叫做<strong>帧</strong>，我们来看下它的数据格式长什么样子。</p>
<p><img src="https://img-blog.csdnimg.cn/img_convert/3a63a86e5d7e72a37b9828fc6e65c21f.png" alt="图片"></p>
<p>这里面字段很多，但我们只需要关注下面这几个。</p>
<p><strong>opcode字段</strong>：这个是用来标志这是个<strong>什么类型</strong>的数据帧。比如。</p>
<ul>
<li>等于 1 ，是指text类型（<code v-pre>string</code>）的数据包</li>
<li>等于 2 ，是二进制数据类型（<code v-pre>[]byte</code>）的数据包</li>
<li>等于 8 ，是关闭连接的信号</li>
</ul>
<p><strong>payload字段</strong>：存放的是我们<strong>真正想要传输的数据的长度</strong>，单位是<strong>字节</strong>。比如你要发送的数据是<code v-pre>字符串&quot;111&quot;</code>，那它的长度就是<code v-pre>3</code>。</p>
<p><img src="https://img-blog.csdnimg.cn/img_convert/437a076935f82be1d36960c9a4785fbd.png" alt="图片"></p>
<p>另外，可以看到，我们存放<strong>payload 长度的字段有好几个</strong>，我们既可以用最前面的<code v-pre>7bit</code>, 也可以用后面的<code v-pre>7+16bit 或 7+64bit。</code></p>
<p>那么问题就来了。</p>
...
<ul>
<li>如果<code v-pre>最开始的7bit</code>的值是 0~125，那么它就表示了 <strong>payload 全部长度</strong>，只读最开始的<code v-pre>7个bit</code>就完事了。</li>
</ul>
<p><img src="https://img-blog.csdnimg.cn/img_convert/690f5a4deda2de50f3a35eddf0be4d75.png" alt="图片"></p>
<ul>
<li>如果是<code v-pre>126（0x7E）</code>。那它表示payload的长度范围在 <code v-pre>126~65535</code> 之间，接下来还需要<strong>再读16bit</strong>。这16bit会包含payload的真实长度。</li>
</ul>
<p><img src="https://img-blog.csdnimg.cn/img_convert/c815c9dabc02fceb42a98c762705af33.png" alt="图片"></p>
<ul>
<li>如果是<code v-pre>127（0x7F）</code>。那它表示payload的长度范围<code v-pre>&gt;=65536</code>，接下来还需要<strong>再读64bit</strong>。这64bit会包含payload的长度。这能放2的64次方byte的数据，换算一下好多个TB，肯定够用了。</li>
</ul>
<p><img src="https://img-blog.csdnimg.cn/img_convert/192b22b4fe46e8dfb7b17549306d5998.png" alt="图片"></p>
...
<p><img src="https://img-blog.csdnimg.cn/img_convert/d449242f1bf41c6f95a5314ec8311d0d.jpeg" alt="图片"></p>
...
<p><img src="https://img-blog.csdnimg.cn/img_convert/b91fedb1856897c231b8fb5932b7b2d2.png" alt="图片"></p>
<h3 id="websocket的使用场景" tabindex="-1"><a class="header-anchor" href="#websocket的使用场景" aria-hidden="true">#</a> WebSocket的使用场景</h3>
...
<p><img src="https://img-blog.csdnimg.cn/img_convert/31410d2e885aab55c2c588aad754bb5c.png" alt="图片"></p>
