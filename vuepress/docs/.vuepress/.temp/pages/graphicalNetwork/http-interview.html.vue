<template><div><h1 id="http-常见面试题" tabindex="-1"><a class="header-anchor" href="#http-常见面试题" aria-hidden="true">#</a> HTTP 常见面试题</h1>
<p>在面试过程中，HTTP 被提问的概率还是比较高的。</p>
<p>我搜集了 6 大类 HTTP 面试常问的题目，同时这 6 大类题跟 <strong>HTTP 的发展和演变</strong>关联性是比较大的，通过<strong>问答 + 图解</strong>的形式<strong>由浅入深</strong>的方式帮助大家进一步的学习和理解 HTTP。</p>
<ol>
<li>HTTP 基本概念</li>
<li>Get 与 Post</li>
<li>HTTP 特性</li>
...
<p><img src="https://img-blog.csdnimg.cn/6b9bfd38d2684b3f9843ebabf8771212.png" alt="提纲"></p>
<h2 id="http-基本概念" tabindex="-1"><a class="header-anchor" href="#http-基本概念" aria-hidden="true">#</a> HTTP 基本概念</h2>
<h3 id="http-是什么" tabindex="-1"><a class="header-anchor" href="#http-是什么" aria-hidden="true">#</a> HTTP 是什么？</h3>
...
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/3-HTTP三部分.png" alt="三个部分"></p>
<p><em>1. 「协议」</em></p>
...
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/4-租房和三方协议.png" alt="三方协议和租房协议"></p>
...
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/5-请求应答.png" alt="请求 - 应答"></p>
<p>数据虽然是在 A 和 B 之间传输，但允许中间有<strong>中转或接力</strong>。</p>
<p>就好像第一排的同学想传递纸条给最后一排的同学，那么传递的过程中就需要经过好多个同学（中间人），这样的传输方式就从「A &lt; --- &gt; B」，变成了「A &lt;-&gt; N &lt;-&gt; M &lt;-&gt; B」。</p>
...
<h3 id="http-常见的状态码有哪些" tabindex="-1"><a class="header-anchor" href="#http-常见的状态码有哪些" aria-hidden="true">#</a> HTTP 常见的状态码有哪些？</h3>
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/6-五大类HTTP状态码.png" alt=" 五大类 HTTP 状态码 "></p>
<p><code v-pre>1xx</code> 类状态码属于<strong>提示信息</strong>，是协议处理中的一种中间状态，实际用到的比较少。</p>
<ul>
<li>「<strong>101 Switching Protocols</strong>」协议切换，服务器已经理解了客户端的请求，并将通过Upgrade消息头通知客户端采用不同的协议来完成这个请求。比如切换到一个实时且同步的协议（如WebSocket）以传送利用此类特性的资源。</li>
</ul>
<p><code v-pre>2xx</code> 类状态码表示服务器<strong>成功</strong>处理了客户端的请求，也是我们最愿意看到的状态。</p>
<ul>
<li>
<p>「<strong>200 OK</strong>」是最常见的成功状态码，表示一切正常。如果是非 <code v-pre>HEAD</code> 请求，服务器返回的响应头都会有 body 数据。</p>
</li>
<li>
...
<ul>
<li>
<p>「<strong>500 Internal Server Error</strong>」与 400 类似，是个笼统通用的错误码，服务器发生了什么错误，我们并不知道。</p>
</li>
<li>
...
<h3 id="http-常见字段有哪些" tabindex="-1"><a class="header-anchor" href="#http-常见字段有哪些" aria-hidden="true">#</a> HTTP 常见字段有哪些？</h3>
<p><em>Host</em> 字段</p>
<p>客户端发送请求时，用来指定服务器的域名。</p>
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/7-HOST字段.png" alt=""></p>
<div class="language-text ext-text line-numbers-mode"><pre v-pre class="language-text"><code>Host: www.A.com
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>有了 <code v-pre>Host</code> 字段，就可以将请求发往「同一台」服务器上的不同网站。</p>
<p><em>Content-Length 字段</em></p>
<p>服务器在返回数据时，会有 <code v-pre>Content-Length</code> 字段，表明本次回应的数据长度。</p>
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/8-content-length字段.png" alt=""></p>
<div class="language-text ext-text line-numbers-mode"><pre v-pre class="language-text"><code>Content-Length: 1000
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>如上面则是告诉浏览器，本次服务器回应的数据长度是 1000 个字节，后面的字节就属于下一个回应了。</p>
<p>大家应该都知道 HTTP 是基于 TCP 传输协议进行通信的，而使用了 TCP 传输协议，就会存在一个“粘包”的问题，<strong>HTTP 协议通过设置回车符、换行符作为 HTTP header 的边界，通过 Content-Length 字段作为 HTTP body 的边界，这两个方式都是为了解决“粘包”的问题</strong>。具体什么是 TCP 粘包，可以看这篇文章：<a href="https://xiaolincoding.com/network/3_tcp/tcp_stream.html" target="_blank" rel="noopener noreferrer">如何理解是 TCP 面向字节流协议？<ExternalLinkIcon/></a></p>
<p><em>Connection 字段</em></p>
<p><code v-pre>Connection</code> 字段最常用于客户端要求服务器使用「HTTP 长连接」机制，以便其他请求复用。</p>
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/9-connection字段.png" alt=""></p>
<p>HTTP 长连接的特点是，只要任意一端没有明确提出断开连接，则保持 TCP 连接状态。</p>
<p><img src="https://img-blog.csdnimg.cn/img_convert/d2b20d1cc03936332adb2a68512eb167.png" alt="HTTP 长连接"></p>
<p>HTTP/1.1 版本的默认连接都是长连接，但为了兼容老版本的 HTTP，需要指定 <code v-pre>Connection</code> 首部字段的值为 <code v-pre>Keep-Alive</code>。</p>
<div class="language-text ext-text line-numbers-mode"><pre v-pre class="language-text"><code>Connection: Keep-Alive
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>开启了 HTTP Keep-Alive 机制后， 连接就不会中断，而是保持连接。当客户端发送另一个请求时，它会使用同一个连接，一直持续到客户端或服务器端提出断开连接。</p>
<p>PS：大家不要把 HTTP  Keep-Alive 和 TCP Keepalive 搞混了，这两个虽然长的像，但是不是一个东西，具体可以看我这篇文章：<a href="https://xiaolincoding.com/network/3_tcp/tcp_http_keepalive.html" target="_blank" rel="noopener noreferrer">TCP Keepalive 和 HTTP Keep-Alive 是一个东西吗？<ExternalLinkIcon/></a></p>
<p><em>Content-Type 字段</em></p>
<p><code v-pre>Content-Type</code> 字段用于服务器回应时，告诉客户端，本次数据是什么格式。</p>
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/10-content-type字段.png" alt=""></p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>上面的类型表明，发送的是网页，而且编码是UTF-8。</p>
<p>客户端请求的时候，可以使用 <code v-pre>Accept</code> 字段声明自己可以接受哪些数据格式。</p>
<div class="language-text ext-text line-numbers-mode"><pre v-pre class="language-text"><code>Accept: */*
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>上面代码中，客户端声明自己可以接受任何格式的数据。</p>
<p><em>Content-Encoding 字段</em></p>
<p><code v-pre>Content-Encoding</code> 字段说明数据的压缩方法。表示服务器返回的数据使用了什么压缩格式</p>
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/11-content-encoding字段.png" alt=""></p>
<div class="language-text ext-text line-numbers-mode"><pre v-pre class="language-text"><code>Content-Encoding: gzip
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>上面表示服务器返回的数据采用了 gzip 方式压缩，告知客户端需要用此方式解压。</p>
<p>客户端在请求时，用 <code v-pre>Accept-Encoding</code> 字段说明自己可以接受哪些压缩方法。</p>
<div class="language-text ext-text line-numbers-mode"><pre v-pre class="language-text"><code>Accept-Encoding: gzip, deflate
...
<h3 id="get-和-post-有什么区别" tabindex="-1"><a class="header-anchor" href="#get-和-post-有什么区别" aria-hidden="true">#</a> GET 和 POST 有什么区别？</h3>
...
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/12-Get请求.png" alt="GET 请求"></p>
...
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/13-Post请求.png" alt="POST 请求"></p>
...
<p>曾经有个笑话，有人写了个博客，删除博客用的是 GET 请求，他觉得没人访问就连鉴权都没做。然后 Google 服务器爬虫爬了一遍，他所有博文就没了。。。</p>
...
<h2 id="http-缓存技术" tabindex="-1"><a class="header-anchor" href="#http-缓存技术" aria-hidden="true">#</a> HTTP 缓存技术</h2>
<h3 id="http-缓存有哪些实现方式" tabindex="-1"><a class="header-anchor" href="#http-缓存有哪些实现方式" aria-hidden="true">#</a> HTTP 缓存有哪些实现方式？</h3>
...
<h3 id="什么是强制缓存" tabindex="-1"><a class="header-anchor" href="#什么是强制缓存" aria-hidden="true">#</a> 什么是强制缓存？</h3>
<p>强缓存指的是只要浏览器判断缓存没有过期，则直接使用浏览器的本地缓存，决定是否使用缓存的主动性在于浏览器这边。</p>
<p>如下图中，返回的是 200 状态码，但在 size 项中标识的是 from disk cache，就是使用了强制缓存。</p>
<p><img src="https://img-blog.csdnimg.cn/1cb6bc37597e4af8adfef412bfc57a42.png" alt=""></p>
...
<h3 id="什么是协商缓存" tabindex="-1"><a class="header-anchor" href="#什么是协商缓存" aria-hidden="true">#</a> 什么是协商缓存？</h3>
...
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/network/http/http缓存.png" alt=""></p>
...
<h2 id="http-特性" tabindex="-1"><a class="header-anchor" href="#http-特性" aria-hidden="true">#</a> HTTP 特性</h2>
<p>到目前为止，HTTP 常见到版本有 HTTP/1.1，HTTP/2.0，HTTP/3.0，不同版本的 HTTP 特性是不一样的。</p>
<p>这里先用  HTTP/1.1 版本给大家介绍，其他版本的后续也会介绍。</p>
<h3 id="http-1-1-的优点有哪些" tabindex="-1"><a class="header-anchor" href="#http-1-1-的优点有哪些" aria-hidden="true">#</a> HTTP/1.1 的优点有哪些？</h3>
<p>HTTP 最突出的优点是「简单、灵活和易于扩展、应用广泛和跨平台」。</p>
<p><em>1. 简单</em></p>
...
<p>同时 HTTP 由于是工作在应用层（ <code v-pre>OSI</code> 第七层），则它<strong>下层可以随意变化</strong>，比如：</p>
<ul>
...
<h3 id="http-1-1-的缺点有哪些" tabindex="-1"><a class="header-anchor" href="#http-1-1-的缺点有哪些" aria-hidden="true">#</a> HTTP/1.1 的缺点有哪些？</h3>
...
<p><code v-pre>Cookie</code> 通过在请求和响应报文中写入 Cookie 信息来控制客户端的状态。</p>
<p>相当于，<strong>在客户端第一次请求后，服务器会下发一个装有客户信息的「小贴纸」，后续客户端请求服务器的时候，带上「小贴纸」，服务器就能认得了了</strong>，</p>
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/14-cookie技术.png" alt="Cookie 技术"></p>
<p><em>2. 明文传输双刃剑</em></p>
...
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/15-你号没了.png" alt=""></p>
<p><em>3. 不安全</em></p>
...
<h3 id="http-1-1-的性能如何" tabindex="-1"><a class="header-anchor" href="#http-1-1-的性能如何" aria-hidden="true">#</a> HTTP/1.1 的性能如何？</h3>
...
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/16-短连接与长连接.png" alt="短连接与长连接"></p>
...
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/17-管道网络传输.png" alt="管道网络传输"></p>
...
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/18-队头阻塞.png" alt="队头阻塞"></p>
...
<h3 id="https-解决了-http-的哪些问题" tabindex="-1"><a class="header-anchor" href="#https-解决了-http-的哪些问题" aria-hidden="true">#</a> HTTPS 解决了 HTTP 的哪些问题？</h3>
...
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/19-HTTPS与HTTP.png" alt="HTTP 与 HTTPS 网络层"></p>
<p>HTTP<strong>S</strong> 在 HTTP 与 TCP 层之间加入了 <code v-pre>SSL/TLS</code> 协议，可以很好的解决了上述的风险：</p>
<ul>
...
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/20-混合加密.png" alt="混合加密"></p>
...
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/摘要算法.png" alt=""></p>
...
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/数字签名.png" alt=""></p>
...
<p><strong>由于通过警察局验证了数字证书是合法的，那么就能证明这个公钥就是你父亲的</strong>，于是老师就可以安心的用这个公钥解密出清教条，如果能解密出，就证明是你爸爸写的请假条。</p>
...
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/22-数字证书工作流程.png" alt="数子证书工作流程"></p>
...
<p>TLS 的「握手阶段」涉及<strong>四次</strong>通信，使用不同的密钥交换算法，TLS 握手流程也会不一样的，现在常用的密钥交换算法有两种：<a href="https://xiaolincoding.com/network/2_http/https_rsa.html" target="_blank" rel="noopener noreferrer">RSA 算法<ExternalLinkIcon/></a> 和 <a href="https://xiaolincoding.com/network/2_http/https_ecdhe.html" target="_blank" rel="noopener noreferrer">ECDHE 算法<ExternalLinkIcon/></a>。</p>
<p>基于 RSA 算法的 TLS 握手过程比较容易理解，所以这里先用这个给大家展示 TLS 握手过程，如下图：</p>
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/23-HTTPS工作流程.png" alt="HTTPS 连接建立过程"></p>
<p>TLS 协议建立的详细流程：</p>
...
<p>服务器收到客户端请求后，向客户端发出响应，也就是 <code v-pre>ServerHello</code>。服务器回应的内容有如下内容：</p>
...
<div class="custom-container tip"><p class="custom-container-title">TIP</p>
<p>如果想深入学习基于 RSA 算法的 HTTPS 握手过程，可以看这篇，我通过抓包的方式，逐步分析每一个过程：<a href="https://xiaolincoding.com/network/2_http/https_rsa.html" target="_blank" rel="noopener noreferrer">HTTPS RSA 握手解析<ExternalLinkIcon/></a></p>
<p>不过，基于 RSA 算法的 HTTPS 存在「前向安全」的问题：如果服务端的私钥泄漏了，过去被第三方截获的所有 TLS 通讯密文都会被破解。</p>
<p>为了解决这个问题，后面就出现了 ECDHE 密钥协商算法，我们现在大多数网站使用的正是 ECDHE 密钥协商算法，关于 ECDHE 握手的过程可以看这篇文章：<a href="https://xiaolincoding.com/network/2_http/https_ecdhe.html#%E7%A6%BB%E6%95%A3%E5%AF%B9%E6%95%B0" target="_blank" rel="noopener noreferrer">HTTPS ECDHE 握手解析<ExternalLinkIcon/></a></p>
</div>
<blockquote>
...
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost4@main/网络/https/证书的校验.png" alt=""></p>
...
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost4@main/网络/https/baidu证书.png" alt=""></p>
...
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost4@main/网络/https/用户信任.png" alt=""></p>
<p>操作系统里一般都会内置一些根证书，比如我的 MAC 电脑里内置的根证书有这么多：</p>
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost4@main/网络/https/系统根证书.png" alt=""></p>
<p>这样的一层层地验证就构成了一条信任链路，整个证书信任链验证流程如下图所示：</p>
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost4@main/网络/https/证书链.png" alt=""></p>
...
<h3 id="https-的应用数据是如何保证完整性的" tabindex="-1"><a class="header-anchor" href="#https-的应用数据是如何保证完整性的" aria-hidden="true">#</a> HTTPS 的应用数据是如何保证完整性的？</h3>
...
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/记录协议.png" alt=""></p>
<p>具体过程如下：</p>
<ul>
<li>
<p>首先，消息被分割成多个较短的片段,然后分别对每个片段进行压缩。</p>
</li>
<li>
<p>接下来，经过压缩的片段会被<strong>加上消息认证码（MAC 值，这个是通过哈希算法生成的），这是为了保证完整性，并进行数据的认证</strong>。通过附加消息认证码的 MAC 值，可以识别出篡改。与此同时，为了防止重放攻击，在计算消息认证码时，还加上了片段的编码。</p>
</li>
<li>
<p>再接下来，经过压缩的片段再加上消息认证码会一起通过对称密码进行加密。</p>
</li>
<li>
<p>最后，上述经过加密的数据再加上由数据类型、版本号、压缩后的长度组成的报头就是最终的报文数据。</p>
</li>
</ul>
<p>记录协议完成后，最终的报文数据将传递到传输控制协议 (TCP) 层进行传输。</p>
<p>如果你想详细了解记录协议是如何分片、压缩、计算 MAC 值、分组加密，可以看这篇：<a href="https://blog.csdn.net/zhanyiwp/article/details/105627799" target="_blank" rel="noopener noreferrer">理解SSL/TLS系列 (四) 记录协议<ExternalLinkIcon/></a></p>
<h3 id="https-一定安全可靠吗" tabindex="-1"><a class="header-anchor" href="#https-一定安全可靠吗" aria-hidden="true">#</a> HTTPS 一定安全可靠吗？</h3>
<p>之前有读者在字节面试的时候，被问到：<strong>HTTPS 一定安全可靠吗？</strong></p>
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/network/http/提问.jpeg" alt=""></p>
<p>这个问题的场景是这样的：客户端通过浏览器向服务端发起 HTTPS 请求时，被「假基站」转发到了一个「中间人服务器」，于是客户端是和「中间人服务器」完成了 TLS 握手，然后这个「中间人服务器」再与真正的服务端完成 TLS 握手。</p>
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/network/http/https中间人.drawio.png" alt=""></p>
<p>具体过程如下：</p>
<ul>
...
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/network/http/证书安全提示.png" alt=""></p>
...
<blockquote>
<p>为什么抓包工具能截取 HTTPS 数据？</p>
</blockquote>
<p>很多抓包工具 之所以可以明文看到 HTTPS 数据，工作原理与中间人一致的。</p>
...
<p>因此使用抓包工具进行 HTTPS 抓包的时候，抓包工具会生成根证书，导入到客户端系统的 受信任的根证书列表 中，这里的根证书实际上起认证中心（CA）的作用。</p>
<p>随后抓包工具使用该根证书签发域名的证书，因为根证书受信任，域名的证书同样会被浏览器信任。也就是抓包工具给自己创建了一个认证中心 CA，客户端拿着中间人（抓包工具）签发的证书去中间人（抓包工具）自己的 CA 做认证，这个证书当然被认为是有效的。</p>
<blockquote>
<p>如何避免被中间人抓取数据？</p>
</blockquote>
...
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/network/http/双向认证.png" alt=""></p>
...
<h3 id="http-2-做了什么优化" tabindex="-1"><a class="header-anchor" href="#http-2-做了什么优化" aria-hidden="true">#</a> HTTP/2 做了什么优化？</h3>
<p>HTTP/2 协议是基于 HTTPS 的，所以 HTTP/2 的安全性也是有保障的。</p>
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/25-HTTP2.png" alt="HTT/1 ~ HTTP/2"></p>
...
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost4@main/网络/http2/二进制帧.png" alt="HTTP/1 与 HTTP/2 "></p>
...
<ol>
<li>最前面的 1 标识该 Header 是静态表中已经存在的 KV。（至于什么是静态表，可以看这篇：<a href="https://xiaolincoding.com/network/2_http/http2.html" target="_blank" rel="noopener noreferrer">HTTP/2 牛逼在哪？<ExternalLinkIcon/></a>）</li>
...
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost4@main/网络/http2/push.png" alt="img"></p>
...
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/network/quic/http2阻塞.jpeg" alt=""></p>
<p>举个例子，如下图：</p>
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost4@main/网络/http3/tcp队头阻塞.gif" alt="img"></p>
...
<div class="custom-container tip"><p class="custom-container-title">TIP</p>
<p>如果想更进一步了解 HTTP/2 协议，可以看我这篇文章：<a href="https://xiaolincoding.com/network/2_http/http2.html" target="_blank" rel="noopener noreferrer">HTTP/2 牛逼在哪？<ExternalLinkIcon/></a></p>
</div>
<h3 id="http-3-做了哪些优化" tabindex="-1"><a class="header-anchor" href="#http-3-做了哪些优化" aria-hidden="true">#</a> HTTP/3 做了哪些优化？</h3>
...
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/27-HTTP3.png" alt="HTTP/1 ~ HTTP/3"></p>
...
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/network/quic/quic无阻塞.jpeg" alt=""></p>
<p><em>2、更快的连接建立</em></p>
<p>对于 HTTP/1 和 HTTP/2 协议，TCP 和 TLS 是分层的，分别属于内核实现的传输层、openssl 库实现的表示层，因此它们难以合并在一起，需要分批次来握手，先 TCP 握手，再 TLS 握手。</p>
<p>HTTP/3 在传输数据前虽然需要 QUIC 协议握手，但这个握手过程只需要 1 RTT，握手的目的是为确认双方的「连接 ID」，连接迁移就是基于连接 ID 实现的。</p>
...
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/28-HTTP3交互次数.png" alt="TCP HTTPS（TLS/1.3） 和 QUIC HTTPS "></p>
...
<p><img src="https://img-blog.csdnimg.cn/4cad213f5125432693e0e2a512c2d1a1.png" alt=""></p>
<p><em>3、连接迁移</em></p>
...
<div class="custom-container tip"><p class="custom-container-title">TIP</p>
<p>如果想更进一步了解 HTTP/3 和 QUIC 协议，可以看我这两篇文章：</p>
<ul>
<li><a href="https://xiaolincoding.com/network/2_http/http3.html" target="_blank" rel="noopener noreferrer">HTTP/3 强势来袭<ExternalLinkIcon/></a></li>
<li><a href="https://xiaolincoding.com/network/3_tcp/quic.html" target="_blank" rel="noopener noreferrer">如何基于 UDP 协议实现可靠传输？<ExternalLinkIcon/></a></li>
</ul>
</div>
<hr>
<p>参考资料：</p>
...
<p>[3] 陈皓.HTTP的前世今.酷壳CoolShell.https://coolshell.cn/articles/19840.html</p>
<p>[4] 阮一峰.HTTP 协议入门.阮一峰的网络日志.http://www.ruanyifeng.com/blog/2016/08/http.html</p>
<hr>
...
<ol>
<li>
<p>建立连接时候：https 比 http 多了 TLS 的握手过程；</p>
</li>
<li>
<p>传输内容的时候：https 会把数据进行加密，通常是对称加密数据；</p>
</li>
</ol>
<blockquote>
...
<p>到了1999年，SSL 因为应用广泛，已经成为互联网上的事实标准。IETF 就在那年把 SSL 标准化。标准化之后的名称改为 TLS（是 “<em>Transport Layer Security</em>” 的缩写），中文叫做 「传输层安全协议」。</p>
...
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/29-TLS1.2-四次握手.png" alt=""></p>
<p>另外， SSL/TLS 1.3 优化了过程，只需要 1 个 RTT 往返时延，也就是只需要 3 次握手：</p>
<p><img src="https://cdn.xiaolincoding.com/gh/xiaolincoder/ImageHost/计算机网络/HTTP/30-TLS1.3.png" alt="T"></p>
<hr>
<p>本文的 <code v-pre>30</code> 张图片，都是从一条线两条线画出来，灰常的费劲，深切感受到画图也是个<strong>体力活</strong>啊！</p>
<p>爱偷懒的我其实不爱画图，但为了让大家能更好的理解，在跟自己无数次斗争后，踏上了耗时耗体力的画图的不归路，希望对你们有帮助！</p>
</div></template>
