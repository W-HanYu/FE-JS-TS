<template><div><h1 id="既然有-http-协议-为什么还要有-rpc" tabindex="-1"><a class="header-anchor" href="#既然有-http-协议-为什么还要有-rpc" aria-hidden="true">#</a> 既然有 HTTP 协议，为什么还要有 RPC？</h1>
<h2 id="从-tcp-聊起" tabindex="-1"><a class="header-anchor" href="#从-tcp-聊起" aria-hidden="true">#</a> 从 TCP 聊起</h2>
...
<p>类似下面这样。</p>
<div class="language-c ext-c line-numbers-mode"><pre v-pre class="language-c"><code>fd <span class="token operator">=</span> <span class="token function">socket</span><span class="token punctuation">(</span>AF_INET<span class="token punctuation">,</span>SOCK_STREAM<span class="token punctuation">,</span><span class="token number">0</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>其中 <code v-pre>SOCK_STREAM</code>，是指使用<strong>字节流</strong>传输数据，说白了就是 <strong>TCP 协议</strong>。</p>
<p>在定义了 Socket 之后，我们就可以愉快的对这个 Socket 进行操作，比如用 <code v-pre>bind()</code> 绑定 IP 端口，用 <code v-pre>connect()</code> 发起建连。</p>
<p><img src="https://img-blog.csdnimg.cn/img_convert/23cc66a7f4cb06afe13842b4b339e28b.gif" alt="握手建立连接流程"></p>
<p>在连接建立之后，我们就可以使用 <code v-pre>send()</code> 发送数据，<code v-pre>recv()</code> 接收数据。</p>
<p>光这样一个纯裸的 TCP 连接，就可以做到收发数据了，那是不是就够了？</p>
<p>不行，这么用会有问题。</p>
<h2 id="使用纯裸-tcp-会有什么问题" tabindex="-1"><a class="header-anchor" href="#使用纯裸-tcp-会有什么问题" aria-hidden="true">#</a> 使用纯裸 TCP 会有什么问题</h2>
<p>八股文常背，TCP 是有三个特点，<strong>面向连接</strong>、<strong>可靠</strong>、基于<strong>字节流</strong>。</p>
<p><img src="https://img-blog.csdnimg.cn/img_convert/3fcad07ba7ae92299b32224da8583363.png" alt="TCP 是什么"></p>
...
<p><img src="https://img-blog.csdnimg.cn/img_convert/254d845f9de05c19536d8343d268595a.png" alt="01 二进制字节流"></p>
<p>正因为这个没有<strong>任何边界</strong>的特点，所以当我们选择使用 TCP 发送&quot;夏洛&quot;和&quot;特烦恼&quot;的时候，接收端收到的就是&quot;夏洛特烦恼&quot;，这时候接收端没发区分你是想要表达&quot;夏洛&quot;+&quot;特烦恼&quot;还是&quot;夏洛特&quot;+&quot;烦恼&quot;。</p>
<p><img src="https://img-blog.csdnimg.cn/img_convert/cd7c006cb4180bf751c4afd268ed44f0.png" alt="消息对比"></p>
<p>这就是所谓的<strong>粘包问题</strong>，之前也写过一篇专门的<a href="https://xiaolincoding.com/network/3_tcp/tcp_stream.html" target="_blank" rel="noopener noreferrer">文章<ExternalLinkIcon/></a>聊过这个问题。</p>
...
<p><img src="https://img-blog.csdnimg.cn/img_convert/9428feed1ff22156fc136d17a129527b.png" alt="消息边界长度标志"></p>
...
<h2 id="http-和-rpc" tabindex="-1"><a class="header-anchor" href="#http-和-rpc" aria-hidden="true">#</a> HTTP 和 RPC</h2>
<p>我们回过头来看网络的分层图。</p>
<p><img src="https://img-blog.csdnimg.cn/img_convert/da970d16a205fb48d6a8bea14498814d.png" alt="四层网络协议"></p>
...
<p><img src="https://img-blog.csdnimg.cn/img_convert/809c33f7090c08b78d494445e39ae1b4.png" alt="HTTP调用"></p>
...
<div class="language-text ext-text line-numbers-mode"><pre v-pre class="language-text"><code> res = localFunc(req)
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>如果现在这不是个本地方法，而是个<strong>远端服务器</strong>暴露出来的一个方法 <code v-pre>remoteFunc</code>，如果我们还能像调用本地方法那样去调用它，这样就可以<strong>屏蔽掉一些网络细节</strong>，用起来更方便，岂不美哉？</p>
<div class="language-text ext-text line-numbers-mode"><pre v-pre class="language-text"><code> res = remoteFunc(req)
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p><img src="https://img-blog.csdnimg.cn/img_convert/2b2ea6d26af9ded517043e528b032307.png" alt="RPC可以像调用本地方法那样调用远端方法"></p>
<p>基于这个思路，大佬们造出了非常多款式的 RPC 协议，比如比较有名的<code v-pre>gRPC</code>，<code v-pre>thrift</code>。</p>
<p>值得注意的是，虽然大部分 RPC 协议底层使用 TCP，但实际上<strong>它们不一定非得使用 TCP，改用 UDP 或者 HTTP，其实也可以做到类似的功能。</strong></p>
<p><img src="https://img-blog.csdnimg.cn/img_convert/054e9738bc492a6fb6e9a71737d95fc0.png" alt="基于TCP协议的HTTP和RPC协议"></p>
<p>到这里，我们回到文章标题的问题。</p>
<blockquote>
<p>既然有 HTTP 协议，为什么还要有 RPC？</p>
</blockquote>
<p>其实，<code v-pre>TCP</code> 是<strong>70年</strong>代出来的协议，而 <code v-pre>HTTP</code> 是 <strong>90 年代</strong>才开始流行的。而直接使用裸 TCP 会有问题，可想而知，这中间这么多年有多少自定义的协议，而这里面就有<strong>80年代</strong>出来的 <code v-pre>RPC</code>。</p>
<p>所以我们该问的不是<strong>既然有 HTTP 协议为什么要有 RPC</strong>，而是<strong>为什么有 RPC 还要有 HTTP 协议</strong>。</p>
<blockquote>
<p>那既然有 RPC 了，为什么还要有 HTTP 呢？</p>
</blockquote>
...
<h2 id="http-和-rpc-有什么区别" tabindex="-1"><a class="header-anchor" href="#http-和-rpc-有什么区别" aria-hidden="true">#</a> HTTP 和 RPC 有什么区别</h2>
...
<h3 id="底层连接形式" tabindex="-1"><a class="header-anchor" href="#底层连接形式" aria-hidden="true">#</a> 底层连接形式</h3>
...
<p><img src="https://img-blog.csdnimg.cn/img_convert/ec5c8e28d3ea308c6db2ac991a12ea80.png" alt="connection_pool"></p>
<p><strong>由于连接池有利于提升网络请求性能，所以不少编程语言的网络库里都会给 HTTP 加个连接池</strong>，比如 <strong>Go</strong> 就是这么干的。</p>
<p>可以看出这一块两者也没太大区别，所以也不是关键。</p>
<h3 id="传输的内容" tabindex="-1"><a class="header-anchor" href="#传输的内容" aria-hidden="true">#</a> 传输的内容</h3>
...
<p><img src="https://img-blog.csdnimg.cn/img_convert/dba2bc3af0938d2c087f85acc191fd3f.png" alt="序列化和反序列化"></p>
...
<p>我们可以随便截个图直观看下。</p>
<p><img src="https://img-blog.csdnimg.cn/img_convert/324cbe84c303a3b975e50329f5cdbf8b.png" alt="HTTP 报文"></p>
<p>可以看到这里面的内容非常多的<strong>冗余</strong>，显得<strong>非常啰嗦</strong>。最明显的，像 <code v-pre>Header</code> 里的那些信息，其实如果我们约定好头部的第几位是 Content-Type，就<strong>不需要每次都真的把&quot;Content-Type&quot;这个字段都传过来</strong>，类似的情况其实在 <code v-pre>body</code> 的 Json 结构里也特别明显。</p>
...
<p><img src="https://img-blog.csdnimg.cn/img_convert/f4cef7331cabcfe56d9d6434f7ef907f.png" alt="HTTP 原理"></p>
<p><img src="https://img-blog.csdnimg.cn/img_convert/12244fb0b19b2e61755fcab799198f68.png" alt="RPC 原理"></p>
<p>当然上面说的 HTTP，其实<strong>特指的是现在主流使用的 HTTP/1.1</strong>，<code v-pre>HTTP/2</code> 在前者的基础上做了很多改进，所以<strong>性能可能比很多 RPC 协议还要好</strong>，甚至连 <code v-pre>gRPC</code> 底层都直接用的 <code v-pre>HTTP/2</code>。</p>
<blockquote>
<p>那么问题又来了，为什么既然有了 HTTP/2，还要有 RPC 协议？</p>
</blockquote>
<p>这个是由于 HTTP/2 是 2015 年出来的。那时候很多公司内部的 RPC 协议都已经跑了好些年了，基于历史原因，一般也没必要去换了。</p>
