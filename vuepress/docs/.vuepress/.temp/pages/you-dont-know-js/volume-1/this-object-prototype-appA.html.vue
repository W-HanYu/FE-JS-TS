<template><div><h1 id="附录-a-es6-class" tabindex="-1"><a class="header-anchor" href="#附录-a-es6-class" aria-hidden="true">#</a> 附录 A: ES6 Class</h1>
<p>如果说本书后半部分（第四到六章）有什么关键信息，那就是类是一种代码的可选设计模式（不是必要的），而且用像 JavaScript 这样的 <code v-pre>[[Prototype]]</code> 语言来实现它总是很尴尬。</p>
<p>虽然这种尴尬很大一部分关于语法，但 <em>不仅</em> 限于此。第四和第五章审视了相当多的难看语法，从使代码杂乱的 <code v-pre>.prototype</code> 引用的繁冗，到 <em>显式假想多态</em>：当你在链条的不同层级上给方法相同的命名以试图实现从低层方法到高层方法的多态引用。<code v-pre>.constructor</code> 被错误地解释为“被 XX 构建”，这成为了一个不可靠的定义，也成为了另一个难看的语法。</p>
<p>但关于类的设计的问题要深刻多了。第四章指出在传统的面向类语言中，类实际上发生了从父类向子类，由子类向实例的 <em>拷贝</em> 动作，而在 <code v-pre>[[Prototype]]</code> 中，动作 <strong>不是</strong> 一个拷贝，而是相反 —— 一个委托链接。</p>
<p>OLOO 风格和行为委托接受了 <code v-pre>[[Prototype]]</code>，而不是将它隐藏起来，当比较它们的简单性时，类在 JS 中的问题就凸显出来。</p>
<h2 id="class" tabindex="-1"><a class="header-anchor" href="#class" aria-hidden="true">#</a> <code v-pre>class</code></h2>
<p>我们 <em>不必</em> 再次争论这些问题。我在这里简单地重提这些问题仅仅是为了使它们在你的头脑里保持新鲜，以使我们将注意力转向 ES6 的 <code v-pre>class</code> 机制。我们将在这里展示它如何工作，并且看看 <code v-pre>class</code> 是否实质上解决了任何这些“类”的问题。</p>
<p>让我们重温第六章的 <code v-pre>Widget</code>/<code v-pre>Button</code> 例子：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token keyword">class</span> <span class="token class-name">Widget</span> <span class="token punctuation">{</span>
  ...
  <span class="token function">render</span><span class="token punctuation">(</span><span class="token parameter">$where</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    <span class="token keyword">if</span> <span class="token punctuation">(</span><span class="token keyword">this</span><span class="token punctuation">.</span>$elem<span class="token punctuation">)</span> <span class="token punctuation">{</span>
      <span class="token keyword">this</span><span class="token punctuation">.</span>$elem
        <span class="token punctuation">.</span><span class="token function">css</span><span class="token punctuation">(</span><span class="token punctuation">{</span>
          ...
          <span class="token literal-property property">height</span><span class="token operator">:</span> <span class="token keyword">this</span><span class="token punctuation">.</span>height <span class="token operator">+</span> <span class="token string">"px"</span><span class="token punctuation">,</span>
        <span class="token punctuation">}</span><span class="token punctuation">)</span>
        <span class="token punctuation">.</span><span class="token function">appendTo</span><span class="token punctuation">(</span>$where<span class="token punctuation">)</span><span class="token punctuation">;</span>
    <span class="token punctuation">}</span>
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
...
<span class="token keyword">class</span> <span class="token class-name">Button</span> <span class="token keyword">extends</span> <span class="token class-name">Widget</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>除了语法上 <em>看起来</em> 更好，ES6 还解决了什么？</p>
<ol>
<li>不再有（某种意义上的，继续往下看！）指向 <code v-pre>.prototype</code> 的引用来弄乱代码。</li>
<li><code v-pre>Button</code> 被声明为直接“继承自”（也就是 <code v-pre>extends</code>）<code v-pre>Widget</code>，而不是需要用 <code v-pre>Object.create(..)</code> 来替换 <code v-pre>.prototype</code> 链接的对象，或者用 <code v-pre>__proto__</code> 和 <code v-pre>Object.setPrototypeOf(..)</code> 来设置它。</li>
<li><code v-pre>super(..)</code> 现在给了我们非常有用的 <strong>相对多态</strong> 的能力，所以在链条上某一个层级上的任何方法，可以引用链条上相对上一层的同名方法。第四章中有一个关于构造器的奇怪现象：构造器不属于它们的类，而且因此与类没有联系。<code v-pre>super(..)</code> 含有一个对此问题的解决方法 —— <code v-pre>super()</code> 会在构造器内部想如你期望的那样工作。</li>
<li><code v-pre>class</code> 字面语法对指定属性没有什么启发（仅对方法有）。这看起来限制了某些东西，但是绝大多数情况下期望一个属性（状态）存在于链条末端的“实例”以外的地方，这通常是一个错误和令人诧异（因为这个状态被隐含地在所有“实例”中“分享”）的。所以，也可以说 <code v-pre>class</code> 语法防止你出现错误。</li>
<li><code v-pre>extends</code> 甚至允许你用非常自然的方式扩展内建的对象（子）类型，比如 <code v-pre>Array</code> 或者 <code v-pre>RegExp</code>。在没有 <code v-pre>class .. extends</code> 的情况下这样做一直以来是一个极端复杂而令人沮丧的任务，只有最熟练的框架作者曾经正确地解决过这个问题。现在，它是小菜一碟！</li>
</ol>
<p>凭心而论，对大多数明显的（语法上的）问题，和经典的原型风格代码使人诧异的地方，这些确实是实质上的解决方案。</p>
<h2 id="class-的坑" tabindex="-1"><a class="header-anchor" href="#class-的坑" aria-hidden="true">#</a> <code v-pre>class</code> 的坑</h2>
<p>然而，它不全是优点。在 JS 中将“类”作为一种设计模式，仍然有一些深刻和非常令人烦恼的问题。</p>
<p>首先，<code v-pre>class</code> 语法可能会说服你 JS 在 ES6 中存在一个新的“类”机制。<strong>但不是这样。</strong> <code v-pre>class</code> 很大程度上仅仅是一个既存的 <code v-pre>[[Prototype]]</code>（委托）机制的语法糖！</p>
<p>这意味着 <code v-pre>class</code> 实际上不是像传统面向类的语言那样，在声明时静态地拷贝定义。如果你在“父类”上更改/替换了一个方法（有意或无意地），子“类”和/或实例将会受到“影响”，因为它们在声明时没有得到一份拷贝，它们依然都使用那个基于 <code v-pre>[[Prototype]]</code> 的实时委托模型。</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token keyword">class</span> <span class="token class-name">C</span> <span class="token punctuation">{</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这种行为只有在 <em>你已经知道了</em> 关于委托的性质，而不是期待从“真的类”中 <em>拷贝</em> 时，才看起来合理。那么你要问自己的问题是，为什么你为了根本上就和类不同的东西选择 <code v-pre>class</code> 语法？</p>
<p>ES6 的 <code v-pre>class</code> 语法不是使观察和理解传统的类和委托对象间的不同 <strong>变得更困难</strong> 了吗？</p>
<p><code v-pre>class</code> 语法 <em>没有</em> 提供声明类的属性成员的方法（仅对方法有）。所以如果你需要跟踪对象间分享的状态，那么你最终会回到难看的 <code v-pre>.prototype</code> 语法，像这样：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token keyword">class</span> <span class="token class-name">C</span> <span class="token punctuation">{</span>
  <span class="token function">constructor</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    <span class="token comment">// 确保修改的是共享状态</span>
    <span class="token comment">// 不是设置实例上的遮蔽属性</span>
    ...
    <span class="token comment">// 这里，`this.count` 通过委托如我们期望的那样工作</span>
...
<span class="token comment">// 直接在原型对象上添加一个共享属性</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这里最大的问题是，由于它将 <code v-pre>.prototype</code> 作为实现细节暴露（泄露！）出来，而背叛了 <code v-pre>class</code> 语法的初衷。</p>
<p>而且，我们还依然面临着那个令人诧异的陷阱：<code v-pre>this.count++</code> 将会隐含地在 <code v-pre>c1</code> 和 <code v-pre>c2</code> 两个对象上创建一个分离的遮蔽属性 <code v-pre>.count</code>，而不是更新共享的状态。<code v-pre>class</code> 没有在这个问题上给我们什么安慰，除了（大概是）通过缺少语法支持来暗示你 <em>根本</em> 就不应该这么做。</p>
<p>另外，无意地遮蔽依然是个灾难：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token keyword">class</span> <span class="token class-name">C</span> <span class="token punctuation">{</span>
  <span class="token function">constructor</span><span class="token punctuation">(</span><span class="token parameter">id</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    <span class="token comment">// 噢，一个坑，我们用实例上的属性值遮蔽了`id()`方法</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>还有一些关于 <code v-pre>super</code> 如何工作的微妙问题。你可能会假设 <code v-pre>super</code> 将会以一种类似与 <code v-pre>this</code> 得到绑定的方式（间第二章）来被绑定，也就是 <code v-pre>super</code> 总是会绑定到当前方法在 <code v-pre>[[Prototype]]</code> 链中的位置的更高一层。</p>
<p>然而，因为性能问题（<code v-pre>this</code> 绑定已经很耗费性能了），<code v-pre>super</code> 不是动态绑定的。它在声明时，被有些“静态地”绑定。不是什么大事儿，对吧？</p>
<p>恩…… 可能是，可能不是。如果你像大多数 JS 开发者那样，开始把函数以各种不同的方式赋值给不同的（来自于 <code v-pre>class</code> 定义的）对象，你可能不会意识到在所有这些情况下，底层的 <code v-pre>super</code> 机制会不得不每次都重新绑定。</p>
<p>而且根据你每次赋值采取的语法方式不同，很有可能在某些情况下 <code v-pre>super</code> 不能被正确地绑定（至少不会像你期望的那样），所以你可能（在写作这里时，TC39 正在讨论这个问题）会不得不用 <code v-pre>toMethod(..)</code> 来手动绑定 <code v-pre>super</code>（有点儿像你不得不用 <code v-pre>bind(..)</code> 绑定 <code v-pre>this</code> —— 见第二章）。</p>
<p>你曾经可以给不同的对象赋予方法，来通过 <em>隐含绑定</em> 规则（见第二章），自动地利用 <code v-pre>this</code> 的动态性。但对于使用 <code v-pre>super</code> 的方法，同样的事情很可能不会发生。</p>
<p>考虑这里 <code v-pre>super</code> 应当怎样动作（对 <code v-pre>D</code> 和 <code v-pre>E</code>）：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token keyword">class</span> <span class="token class-name">P</span> <span class="token punctuation">{</span>
  <span class="token function">foo</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    console<span class="token punctuation">.</span><span class="token function">log</span><span class="token punctuation">(</span><span class="token string">"P.foo"</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
...
<span class="token keyword">var</span> <span class="token constant">D</span> <span class="token operator">=</span> <span class="token punctuation">{</span>
  <span class="token function-variable function">foo</span><span class="token operator">:</span> <span class="token keyword">function</span> <span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    console<span class="token punctuation">.</span><span class="token function">log</span><span class="token punctuation">(</span><span class="token string">"D.foo"</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
  <span class="token punctuation">}</span><span class="token punctuation">,</span>
<span class="token punctuation">}</span><span class="token punctuation">;</span>
...
<span class="token keyword">var</span> <span class="token constant">E</span> <span class="token operator">=</span> <span class="token punctuation">{</span>
  <span class="token literal-property property">foo</span><span class="token operator">:</span> <span class="token class-name">C</span><span class="token punctuation">.</span>prototype<span class="token punctuation">.</span>foo<span class="token punctuation">,</span>
<span class="token punctuation">}</span><span class="token punctuation">;</span>

<span class="token comment">// E 链接到 D 来进行委托</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如果你（十分合理地！）认为 <code v-pre>super</code> 将会在调用时自动绑定，你可能会期望 <code v-pre>super()</code> 将会自动地认识到 <code v-pre>E</code> 委托至 <code v-pre>D</code>，所以使用 <code v-pre>super()</code> 的 <code v-pre>E.foo()</code> 应当调用 <code v-pre>D.foo()</code>。</p>
<p><strong>不是这样。</strong> 由于实用主义的性能原因，<code v-pre>super</code> 不像 <code v-pre>this</code> 那样 <em>延迟绑定</em>（也就是动态绑定）。相反它从调用时 <code v-pre>[[HomeObject]].[[Prototype]]</code> 派生出来，而 <code v-pre>[[HomeObject]]</code> 是在声明时静态绑定的。</p>
<p>在这个特定的例子中，<code v-pre>super()</code> 依然解析为 <code v-pre>P.foo()</code>，因为方法的 <code v-pre>[[HomeObject]]</code> 仍然是 <code v-pre>C</code> 而且 <code v-pre>C.[[Prototype]]</code> 是 <code v-pre>P</code>。</p>
<p><em>可能</em> 会有方法手动地解决这样的陷阱。在这个场景中使用 <code v-pre>toMethod(..)</code> 来绑定/重绑定方法的 <code v-pre>[[HomeObject]]</code>（设置这个对象的 <code v-pre>[[Prototype]]</code> 一起！）似乎会管用：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token keyword">var</span> <span class="token constant">D</span> <span class="token operator">=</span> <span class="token punctuation">{</span>
  <span class="token function-variable function">foo</span><span class="token operator">:</span> <span class="token keyword">function</span> <span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    console<span class="token punctuation">.</span><span class="token function">log</span><span class="token punctuation">(</span><span class="token string">"D.foo"</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
  <span class="token punctuation">}</span><span class="token punctuation">,</span>
<span class="token punctuation">}</span><span class="token punctuation">;</span>
...
<span class="token comment">// 手动绑定 `foo` 的 `[[HomeObject]]` 到</span>
<span class="token comment">// `E`, 因为 `E.[[Prototype]]` 是 `D`，所以</span>
<span class="token comment">// `super()` 是 `D.foo()`</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>注意：</strong> <code v-pre>toMethod()</code> 克隆这个方法，然后将它的第一个参数作为 <code v-pre>homeObject</code>（这就是为什么我们传入 <code v-pre>E</code>），第二个参数（可选）用来设置新方法的 <code v-pre>name</code>（保持“foo”不变）。</p>
<p>除了这种场景以外，是否还有其他的极端情况会使开发者们落入陷阱还有待观察。无论如何，你将不得不费心保持清醒：在哪里引擎自动为你确定 <code v-pre>super</code>，和在哪里你不得不手动处理它。<strong>噢！</strong></p>
<h1 id="静态优于动态" tabindex="-1"><a class="header-anchor" href="#静态优于动态" aria-hidden="true">#</a> 静态优于动态？</h1>
<p>但是关于 ES6 的最大问题是，所有这些种种陷阱意味着 <code v-pre>class</code> 有点儿将你带入一种语法，它看起来暗示着（像传统的类那样）一旦你声明一个 <code v-pre>class</code>，它是一个东西的静态定义（将来会实例化）。使你完全忘记了这个事实：<code v-pre>C</code> 是一个对象，一个你可以直接互动的具体的东西。</p>
