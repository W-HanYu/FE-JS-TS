<template><div><h1 id="第5章-原型-prototype" tabindex="-1"><a class="header-anchor" href="#第5章-原型-prototype" aria-hidden="true">#</a> 第5章: 原型（Prototype）</h1>
<p>在第3、4章中，我们三番五次提到了 <code v-pre>[[Prototype]]</code> 链，但我们并没有讨论它到底是什么。现在我们就详细讲解一下它。</p>
<p><strong>注意：</strong> 所有模拟类拷贝行为的方法，如各种<code v-pre>mixin</code>，并没有使用<code v-pre>[[Prototype]]机制</code></p>
<h2 id="prototype" tabindex="-1"><a class="header-anchor" href="#prototype" aria-hidden="true">#</a> [[Prototype]]</h2>
<p>JavaScript 中的对象有一个内部属性，在语言规范中称为 <code v-pre>[[Prototype]]</code>，它只是一个其他对象的引用。几乎所有的对象在被创建时，该属性都被赋予了一个非空值。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>[[Prototype]]</code> 引用有什么用？在第三章中，我们讲解了 <code v-pre>[[Get]]</code> 操作，它会在你引用一个对象上的属性时被调用，比如 <code v-pre>myObject.a</code>。对于默认的 <code v-pre>[[Get]]</code> 操作来说，第一步就是检查对象本身是否拥有一个 <code v-pre>a</code> 属性，如果有，就使用它。</p>
...
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token keyword">var</span> anotherObject <span class="token operator">=</span> <span class="token punctuation">{</span>
  <span class="token literal-property property">a</span><span class="token operator">:</span> <span class="token number">2</span><span class="token punctuation">,</span>
<span class="token punctuation">}</span><span class="token punctuation">;</span>

<span class="token comment">// 创建一个链接到 `anotherObject` 的对象</span>
...
<span class="token comment">// 找到: a</span>
...
<h3 id="object-prototype" tabindex="-1"><a class="header-anchor" href="#object-prototype" aria-hidden="true">#</a> Object.prototype</h3>
...
<h3 id="属性设置与遮蔽" tabindex="-1"><a class="header-anchor" href="#属性设置与遮蔽" aria-hidden="true">#</a> 属性设置与遮蔽</h3>
<p>回到第三章，我们提到过在对象上设置属性要比仅仅在对象上添加新属性或改变既存属性的值更加微妙。现在我们将更完整地重温这个话题。</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code>myObject<span class="token punctuation">.</span>foo <span class="token operator">=</span> <span class="token string">"bar"</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>如果 <code v-pre>myObject</code> 对象已经直接拥有了普通的名为 <code v-pre>foo</code> 的数据访问器属性，那么这个赋值就和改变既存属性的值一样简单。</p>
...
myObject<span class="token punctuation">.</span>a<span class="token operator">++</span><span class="token punctuation">;</span> <span class="token comment">// 噢，隐式遮蔽！</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>虽然看起来 <code v-pre>myObject.a++</code> 应当（通过委托）查询并 <em>原地</em> 递增 <code v-pre>anotherObject.a</code> 属性，但是 <code v-pre>++</code> 操作符相当于 <code v-pre>myObject.a = myObject.a + 1</code>。结果就是在 <code v-pre>[[Prototype]]</code> 上进行 <code v-pre>a</code> 的 <code v-pre>[[Get]]</code> 查询，从 <code v-pre>anotherObject.a</code> 得到当前的值 <code v-pre>2</code>，将这个值递增 1，然后将值 <code v-pre>3</code> 用 <code v-pre>[[Put]]</code> 赋值到 <code v-pre>myObject</code> 上的新遮蔽属性 <code v-pre>a</code> 上。噢！</p>
...
<h3 id="类-函数" tabindex="-1"><a class="header-anchor" href="#类-函数" aria-hidden="true">#</a> “类”函数</h3>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>当通过调用 <code v-pre>new Foo()</code> 创建 <code v-pre>a</code> 时，会发生的事情之一（见第二章了解所有 <em>四个</em> 步骤）是，<code v-pre>a</code> 得到一个内部 <code v-pre>[[Prototype]]</code> 链接，此链接链到 <code v-pre>Foo.prototype</code> 所指向的对象。</p>
<p>停一会来思考一下这句话的含义。</p>
...
<p>但是在 JavaScript 中，没有这样的拷贝处理发生。你不会创建类的多个实例。你可以创建多个对象，它们的 <code v-pre>[[Prototype]]</code> 连接至一个共通对象。但默认地，没有拷贝发生，如此这些对象彼此间最终不会完全分离和切断关系，而是 <strong><em>链接在一起</em></strong>。</p>
...
<h4 id="名称的意义何在" tabindex="-1"><a class="header-anchor" href="#名称的意义何在" aria-hidden="true">#</a> 名称的意义何在？</h4>
<p>在 JavaScript 中，我们不从一个对象（“类”）向另一个对象（“实例”） <em>拷贝</em>。我们在对象之间制造 <em>链接</em>。对于 <code v-pre>[[Prototype]]</code> 机制，视觉上，箭头的移动方向是从右至左，由下至上。</p>
<img src="@source/you-dont-know-js/assets/this-object-prototypes-fig5.png">
<p>这种机制常被称为“原型继承（prototypal inheritance）”（我们很快就用代码说明），它经常被说成是动态语言版的“类继承”。这种说法试图建立在面向类世界中对“继承”含义的共识上。但是 <em>弄拧</em>（<strong>意思是：抹平</strong>） 了被理解的语义，来适应动态脚本。</p>
<p>先入为主，“继承”这个词有很强烈的含义（见第四章）。仅仅在它前面加入“原型”来区别于 JavaScript 中 <em>实际上几乎相反</em> 的行为，使真相在泥泞般的困惑中沉睡了近二十年。</p>
<p>我想说，将“原型”贴在“继承”之前很大程度上搞反了它的实际意义，就像一只手拿着一个桔子，另一手拿着一个苹果，而坚持说苹果是一个“红色的桔子”。无论我在它前面放什么令人困惑的标签，那都不会改变一个水果是苹果而另一个是桔子的 <em>事实</em>。</p>
<p>更好的方法是直白地将苹果称为苹果——使用最准确和最直接的术语。这样能更容易地理解它们的相似之处和 <strong>许多不同之处</strong>，因为我们都对“苹果”的意义有一个简单的，共享的理解。</p>
<p>由于用语的模糊和歧义，我相信，对于解释 JavaScript 机制真正如何工作来说，“原型继承”这个标签（以及试图错误地应用所有面向类的术语，比如“类”，“构造器”，“实例”，“多态”等）本身带来的 <strong>危害比好处多</strong>。</p>
<p>“继承”意味着 <em>拷贝</em> 操作，而 JavaScript 不拷贝对象属性（原生上，默认地）。相反，JS 在两个对象间建立链接，一个对象实质上可以将对属性/函数的访问 <em>委托</em> 到另一个对象上。对于描述 JavaScript 对象链接机制来说，“委托”是一个准确得多的术语。</p>
<p>另一个有时被扔到 JavaScript 旁边的术语是“差分继承”。它的想法是，我们可以用一个对象与一个更泛化的对象的 <em>不同</em> 来描述一个它的行为。比如，你要解释汽车是一种载具，与其重新描述组成一个一般载具的所有特点，不如只说它有四个轮子。</p>
<p>如果你试着想象，在 JS 中任何给定的对象都是通过委托可用的所有行为的总和，而且 <strong>在你思维中你扁平化</strong> 所有的行为到一个有形的 <em>东西</em> 中，那么你就可以（八九不离十地）看到“差分继承”是如何自圆其说的。</p>
<p>但正如“原型继承”，“差分继承”假意使你的思维模型比在语言中物理发生的事情更重要。它忽视了这样一个事实：对象 <code v-pre>B</code> 实际上不是一个差异结构，而是由一些定义好的特定性质，与一些没有任何定义的“漏洞”组成的。正是通过这些“漏洞”（缺少定义），委托可以接管并且动态地用委托行为“填补”它们。</p>
<p>对象不是像“差分继承”的思维模型所暗示的那样，原生默认地，<strong>通过拷贝</strong> 扁平化到一个单独的差异对象中。因此，对于描述 JavaScript 的 <code v-pre>[[Prototype]]</code> 机制如何工作来说，“差分继承”就不是自然合理。</p>
<p>你 <em>可以选择</em> 偏向“差分继承”这个术语和思维模型，这是个人口味的问题，但是不能否认这个事实：它 <em>仅仅</em> 符合你思维中的主观过程，不是引擎的物理行为。</p>
<h3 id="构造函数-constructors" tabindex="-1"><a class="header-anchor" href="#构造函数-constructors" aria-hidden="true">#</a> &quot;构造函数&quot;（Constructors）</h3>
<p>让我们回到早先的代码：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token keyword">function</span> <span class="token function">Foo</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  <span class="token comment">// ...</span>
<span class="token punctuation">}</span>
...
<span class="token keyword">var</span> a <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">Foo</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>到底是什么导致我们认为 <code v-pre>Foo</code> 是一个“类”？</p>
<p>其一，我们看到了 <code v-pre>new</code> 关键字的使用，就像面向类语言中人们构建类的对象那样。另外，它看起来我们事实上执行了一个类的 <em>构造器</em> 方法，因为 <code v-pre>Foo()</code> 实际上是个被调用的方法，就像当你初始化一个真实的类时这个类的构造器被调用的那样。</p>
<p>为了使“构造器”的语义更令人糊涂，被随意贴上标签的 <code v-pre>Foo.prototype</code> 对象还有另外一招。考虑这段代码：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token keyword">function</span> <span class="token function">Foo</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  <span class="token comment">// ...</span>
<span class="token punctuation">}</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>Foo.prototype</code> 对象默认地（就在代码段中第一行中声明的地方！）得到一个公有的，称为 <code v-pre>.constructor</code> 的不可枚举（见第三章）属性，而且这个属性回头指向这个对象关联的函数（这里是 <code v-pre>Foo</code>）。另外，我们看到被“构造器”调用 <code v-pre>new Foo()</code> 创建的对象 <code v-pre>a</code> <em>看起来</em> 也拥有一个称为 <code v-pre>.constructor</code> 的属性，也相似地指向“创建它的函数”。</p>
<p><strong>注意：</strong> 这实际上不是真的。<code v-pre>a</code> 上没有 <code v-pre>.constructor</code> 属性，而 <code v-pre>a.constructor</code> 确实解析成了 <code v-pre>Foo</code> 函数，“constructor”并不像它看起来的那样实际意味着“被 XX 创建”。我们很快就会解释这个奇怪的地方。</p>
<p>哦，是的，另外…… 根据 JavaScript 世界中的惯例，“类”都以大写字母开头的单词命名，所以使用 <code v-pre>Foo</code> 而不是 <code v-pre>foo</code> 强烈地意味着我们打算让它成为一个“类”。这对你来说太明显了，对吧！？</p>
<p><strong>注意：</strong> 这个惯例是如此强大，以至于如果你在一个小写字母名称的方法上使用 <code v-pre>new</code> 调用，或并没有在一个大写字母开头的函数上使用 <code v-pre>new</code>，许多 JS 语法检查器将会报告错误。这是因为我们如此努力地想要在 JavaScript 中将（假的）“面向类” <em>搞对</em>，所以我们建立了这些语法规则来确保我们使用了大写字母，即便对 JS 引擎来讲，大写字母根本没有 <em>任何意义</em>。</p>
<h4 id="构造器还是调用" tabindex="-1"><a class="header-anchor" href="#构造器还是调用" aria-hidden="true">#</a> 构造器还是调用？</h4>
<p>上面的代码的段中，我们试图认为 <code v-pre>Foo</code> 是一个“构造器”，是因为我们用 <code v-pre>new</code> 调用它，而且我们观察到它“构建”了一个对象。</p>
<p>在现实中，<code v-pre>Foo</code> 不会比你的程序中的其他任何函数“更像构造器”。函数自身 <strong>不是</strong> 构造器。但是，当你在普通函数调用前面放一个 <code v-pre>new</code> 关键字时，这就将函数调用变成了“构造器调用”。事实上，<code v-pre>new</code> 在某种意义上劫持了普通函数并将它以另一种方式调用：构建一个对象，<strong>外加这个函数要做的其他任何事</strong>。</p>
<p>举个例子:</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token keyword">function</span> <span class="token function">NothingSpecial</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<p>换句话说，在 JavaScrip t 中，更合适的说法是，“构造器”是在前面 <strong>用 <code v-pre>new</code> 关键字调用的任何函数</strong>。</p>
<p>函数不是构造器，但是当且仅当 <code v-pre>new</code> 被使用时，函数调用是一个“构造器调用”。</p>
<h3 id="技术" tabindex="-1"><a class="header-anchor" href="#技术" aria-hidden="true">#</a> 技术</h3>
<p>仅仅是这些原因使得 JavaScript 中关于“类”的讨论变得命运多舛吗？</p>
<p><strong>不全是。</strong> JS 开发者们努力尽可能地模拟面向类：</p>
...
b<span class="token punctuation">.</span><span class="token function">myName</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span> <span class="token comment">// "b"</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这段代码展示了另外两种“面向类”的花招：</p>
<ol>
<li>
<p><code v-pre>this.name = name</code>：在每个对象（分别在 <code v-pre>a</code> 和 <code v-pre>b</code> 上；参照第二章关于 <code v-pre>this</code> 绑定的内容）上添加了 <code v-pre>.name</code> 属性，和类的实例包装数据值很相似。</p>
</li>
<li>
<p><code v-pre>Foo.prototype.myName = ...</code>：这也许是更有趣的技术，它在 <code v-pre>Foo.prototype</code> 对象上添加了一个属性（函数）。现在，也许让人惊奇，<code v-pre>a.myName()</code> 可以工作。但是是如何工作的？</p>
</li>
</ol>
<p>在上面的代码段中，有很强的倾向认为当 <code v-pre>a</code> 和 <code v-pre>b</code> 被创建时，<code v-pre>Foo.prototype</code> 上的属性/函数被 <em>拷贝</em> 到了 <code v-pre>a</code> 与 <code v-pre>b</code> 俩个对象上。<strong>但是，这没有发生。</strong></p>
<p>在本章开头，我们解释了 <code v-pre>[[Prototype]]</code> 链，以及它如何作为默认的 <code v-pre>[[Get]]</code> 算法的一部分，在不能直接在对象上找到属性引用时提供后备的查询步骤。</p>
<p>于是，得益于他们被创建的方式，<code v-pre>a</code> 和 <code v-pre>b</code> 都最终拥有一个内部的 <code v-pre>[[Prototype]]</code> 链接链到 <code v-pre>Foo.prototype</code>。当无法分别在 <code v-pre>a</code> 和 <code v-pre>b</code> 中找到 <code v-pre>myName</code> 时，就会在 <code v-pre>Foo.prototype</code> 上找到（通过委托，见第六章）。</p>
<h4 id="复活-构造器" tabindex="-1"><a class="header-anchor" href="#复活-构造器" aria-hidden="true">#</a> 复活“构造器”</h4>
...
<span class="token class-name">Foo</span><span class="token punctuation">.</span>prototype <span class="token operator">=</span> <span class="token punctuation">{</span>
  <span class="token comment">/* .. */</span>
<span class="token punctuation">}</span><span class="token punctuation">;</span> <span class="token comment">// 创建一个新的 prototype 对象</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>Object(..)</code> 没有“构建” <code v-pre>a1</code>，是吧？看起来确实是 <code v-pre>Foo()</code> “构建了”它。许多开发者认为 <code v-pre>Foo()</code> 在执行构建，但当你认为“构造器”意味着“被 XX 构建”时，一切就都崩塌了，因为如果那样的话，<code v-pre>a1.constructor</code> 应当是 <code v-pre>Foo</code>，但它不是！</p>
...
<span class="token comment">// 需要正确地“修复”丢失的 `.constructor`</span>
<span class="token comment">// 新对象上的属性以 `Foo.prototype` 的形式提供。</span>
<span class="token comment">// `defineProperty(..)` 的内容见第三章。</span>
Object<span class="token punctuation">.</span><span class="token function">defineProperty</span><span class="token punctuation">(</span><span class="token class-name">Foo</span><span class="token punctuation">.</span>prototype<span class="token punctuation">,</span> <span class="token string">"constructor"</span><span class="token punctuation">,</span> <span class="token punctuation">{</span>
  ...
  <span class="token literal-property property">value</span><span class="token operator">:</span> Foo<span class="token punctuation">,</span> <span class="token comment">// 使 `.constructor` 指向 `Foo`</span>
<span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>修复 <code v-pre>.constructor</code> 要花不少功夫。而且，我们做的一切是为了延续“构造器”意味着“被 XX 构建”的误解。这是一种昂贵的假象。</p>
<p>事实上，一个对象上的 <code v-pre>.constructor</code> 默认地随意指向一个函数，而这个函数反过来拥有一个指向被这个对象称为 <code v-pre>.prototype</code> 的对象。“构造器”和“原型”这两个词仅有松散的默认含义，可能是真的也可能不是真的。最佳方案是提醒你自己，“构造器不是意味着被 XX 构建”。</p>
...
<h2 id="原型-继承" tabindex="-1"><a class="header-anchor" href="#原型-继承" aria-hidden="true">#</a> (原型) 继承</h2>
...
<img src="@source/you-dont-know-js/assets/this-object-prototypes-fig4.png">
<p>回想之前这幅图，它不仅展示了从对象（也就是“实例”）<code v-pre>a1</code> 到对象 <code v-pre>Foo.prototype</code> 的委托，而且从 <code v-pre>Bar.prototype</code> 到 <code v-pre>Foo.prototype</code>，这酷似类继承的亲子概念。<em>酷似</em>，除了方向，箭头表示的是委托链接，而不是拷贝操作。</p>
...
<span class="token comment">// 这里，我们创建一个新的 `Bar.prototype` 链接链到 `Foo.prototype`</span>
...
<span class="token comment">// 注意！现在 `Bar.prototype.constructor` 不存在了，</span>
<span class="token comment">// 如果你有依赖这个属性的习惯的话，它可以被手动“修复”。</span>
...
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token comment">// 不会如你期望的那样工作!</span>
...
<span class="token comment">// 会如你期望的那样工作</span>
<span class="token comment">// 但会带有你可能不想要的副作用 :(</span>
...
<p>如果有一种标准且可靠地方法来修改既存对象的链接就好了。ES6 之前，有一个非标准的，而且不是完全对所有浏览器通用的方法：通过可以设置的 <code v-pre>.__proto__</code> 属性。ES6 中增加了 <code v-pre>Object.setPrototypeOf(..)</code> 辅助工具，它提供了标准且可预见的方法。</p>
<p>让我们一对一地比较一下 ES6 之前和 ES6 标准的技术如何处理将 <code v-pre>Bar.prototype</code> 链接至 <code v-pre>Foo.prototype</code>：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token comment">// ES6 以前</span>
<span class="token comment">// 扔掉默认既存的 `Bar.prototype`</span>
<span class="token class-name">Bar</span><span class="token punctuation">.</span>prototype <span class="token operator">=</span> Object<span class="token punctuation">.</span><span class="token function">create</span><span class="token punctuation">(</span><span class="token class-name">Foo</span><span class="token punctuation">.</span>prototype<span class="token punctuation">)</span><span class="token punctuation">;</span>

<span class="token comment">// ES6+</span>
<span class="token comment">// 修改既存的 `Bar.prototype`</span>
...
<h3 id="检查-类-关系" tabindex="-1"><a class="header-anchor" href="#检查-类-关系" aria-hidden="true">#</a> 检查“类”关系</h3>
...
<span class="token keyword">var</span> a <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">Foo</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>那么我们如何自省 <code v-pre>a</code> 来找到它的“祖先”（委托链）呢？一种方式是接受“类”的困惑：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code>a <span class="token keyword">instanceof</span> <span class="token class-name">Foo</span><span class="token punctuation">;</span> <span class="token comment">// true</span>
...
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token comment">// 用来检查 `o1` 是否关联到（委托至）`o2` 的帮助函数</span>
...
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token class-name">Foo</span><span class="token punctuation">.</span>prototype<span class="token punctuation">.</span><span class="token function">isPrototypeOf</span><span class="token punctuation">(</span>a<span class="token punctuation">)</span><span class="token punctuation">;</span> <span class="token comment">// true</span>
...
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token comment">// 简单地：`b` 在 `c` 的 `[[Prototype]]` 链中出现过吗？</span>
b<span class="token punctuation">.</span><span class="token function">isPrototypeOf</span><span class="token punctuation">(</span>c<span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>注意，这种方法根本不要求有一个函数（“类”）。它仅仅使用对象的直接引用 <code v-pre>b</code> 和 <code v-pre>c</code>，来查询他们的关系。换句话说，我们上面的 <code v-pre>isRelatedTo(..)</code> 工具是内建在语言中的，它的名字叫 <code v-pre>isPrototypeOf(..)</code>。</p>
<p>我们也可以直接取得一个对象的 <code v-pre>[[Prototype]]</code>。在 ES5 中，这么做的标准方法是：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code>Object<span class="token punctuation">.</span><span class="token function">getPrototypeOf</span><span class="token punctuation">(</span>a<span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>而且你将注意到对象引用是我们期望的：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code>Object<span class="token punctuation">.</span><span class="token function">getPrototypeOf</span><span class="token punctuation">(</span>a<span class="token punctuation">)</span> <span class="token operator">===</span> <span class="token class-name">Foo</span><span class="token punctuation">.</span>prototype<span class="token punctuation">;</span> <span class="token comment">// true</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>大多数浏览器（不是全部！）还一种长期支持的，非标准方法可以访问内部的 <code v-pre>[[Prototype]]</code>：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code>a<span class="token punctuation">.</span>__proto__ <span class="token operator">===</span> <span class="token class-name">Foo</span><span class="token punctuation">.</span>prototype<span class="token punctuation">;</span> <span class="token comment">// true</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>这个奇怪的 <code v-pre>.__proto__</code>（直到 ES6 才被标准化！）属性“魔法般地”取得一个对象内部的 <code v-pre>[[Prototype]]</code> 作为引用，如果你想要直接考察（甚至遍历：<code v-pre>.__proto__.__proto__...</code>）<code v-pre>[[Prototype]]</code> 链，这个引用十分有用。</p>
...
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code>Object<span class="token punctuation">.</span><span class="token function">defineProperty</span><span class="token punctuation">(</span><span class="token class-name">Object</span><span class="token punctuation">.</span>prototype<span class="token punctuation">,</span> <span class="token string">"__proto__"</span><span class="token punctuation">,</span> <span class="token punctuation">{</span>
  ...
  <span class="token function-variable function">set</span><span class="token operator">:</span> <span class="token keyword">function</span> <span class="token punctuation">(</span><span class="token parameter">o</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    <span class="token comment">// ES6 的 setPrototypeOf(..)</span>
...
<p><strong>注意：</strong> 在 ES6 中，关键字 <code v-pre>class</code> 将允许某些近似方法，对像 <code v-pre>Array</code> 这样的内建类型“定义子类”。参见附录 A 中关于 ES6 中加入的 <code v-pre>class</code> 的讨论。</p>
...
<h2 id="对象链接" tabindex="-1"><a class="header-anchor" href="#对象链接" aria-hidden="true">#</a> 对象链接</h2>
<p>正如我们看到的，<code v-pre>[[Prototype]]</code> 机制是一个内部链接，它存在于一个对象上，这个对象引用一些其他的对象。</p>
<p>这种链接（主要）在对一个对象进行属性/方法引用，但这样的属性/方法不存在时实施。在这种情况下，<code v-pre>[[Prototype]]</code> 链接告诉引擎在那个被链接的对象上查找这个属性/方法。接下来，如果这个对象不能满足查询，它的 <code v-pre>[[Prototype]]</code> 又会被查找，如此继续。这个在对象间的一系列链接构成了所谓的“原形链”。</p>
<h3 id="创建链接" tabindex="-1"><a class="header-anchor" href="#创建链接" aria-hidden="true">#</a> 创建链接</h3>
...
<h4 id="填补-object-create" tabindex="-1"><a class="header-anchor" href="#填补-object-create" aria-hidden="true">#</a> 填补 <code v-pre>Object.create()</code></h4>
...
<span class="token keyword">var</span> myObject <span class="token operator">=</span> <span class="token function">createAndLinkObject</span><span class="token punctuation">(</span>anotherObject<span class="token punctuation">)</span><span class="token punctuation">;</span>

myObject<span class="token punctuation">.</span>a<span class="token punctuation">;</span> <span class="token comment">// 2</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>我不会分享这种严格的观点。我完全拥护如上面展示的 <code v-pre>Object.create(..)</code> 的常见部分填补，甚至在 ES5 之前的环境下在你的代码中使用它。我将选择权留给你。</p>
<h3 id="链接作为候补" tabindex="-1"><a class="header-anchor" href="#链接作为候补" aria-hidden="true">#</a> 链接作为候补？</h3>
...
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token keyword">var</span> anotherObject <span class="token operator">=</span> <span class="token punctuation">{</span>
  <span class="token function-variable function">cool</span><span class="token operator">:</span> <span class="token keyword">function</span> <span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    console<span class="token punctuation">.</span><span class="token function">log</span><span class="token punctuation">(</span><span class="token string">"cool!"</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
  <span class="token punctuation">}</span><span class="token punctuation">,</span>
<span class="token punctuation">}</span><span class="token punctuation">;</span>

<span class="token keyword">var</span> myObject <span class="token operator">=</span> Object<span class="token punctuation">.</span><span class="token function">create</span><span class="token punctuation">(</span>anotherObject<span class="token punctuation">)</span><span class="token punctuation">;</span>

myObject<span class="token punctuation">.</span><span class="token function">cool</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span> <span class="token comment">// "cool!"</span>
...
myObject<span class="token punctuation">.</span><span class="token function-variable function">doCool</span> <span class="token operator">=</span> <span class="token keyword">function</span> <span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  <span class="token keyword">this</span><span class="token punctuation">.</span><span class="token function">cool</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span> <span class="token comment">// internal delegation!</span>
<span class="token punctuation">}</span><span class="token punctuation">;</span>

myObject<span class="token punctuation">.</span><span class="token function">doCool</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span> <span class="token comment">// "cool!"</span>
