...
至此，你应当对作用域，以及变量如何根据它们被声明的位置和方式将变量分配给作用域的相关原理很熟悉了啦。函数作用域和块作用域的行为都是依赖于这个相同规则的：在一个作用域中声明的任何变量都附着在这个作用域上。
...
所以，关于这种处理的一个有些隐喻的考虑方式是，_变量和函数声明被从它们在代码流中出现的位置“移动”到代码的顶端。这就产生了“提升”这个名字_。

换句话说，**先有蛋（`声明`），后有鸡（`赋值`）**。

**注意：** 只有声明本身被提升了，而任何赋值或者其他的执行逻辑都被留在 _原处_。如果提升会重新安排我们代码的可执行逻辑，那就会是`“重大事故”`了。
...
还需要注意的是，提升是 **`以作用域为单位的`**。所以虽然我们的前一个代码段被简化为仅含有全局作用域，但是我们现在讨论的函数`foo(..)`本身展示了，`var a`被提升至`foo(..)`的顶端（很明显，不是程序的顶端）。所以这个程序也许可以更准确地解释为：
